...
Los tests cubren el modelo `Direccion` (cálculo de distancias, `__str__`) y el flujo de login (acceso a `/`, login válido e inválido).
...
- **Seguridad**: el proyecto se entrega listo para desarrollo (`DEBUG = True`). Para producción, cambiar `SECRET_KEY`, desactivar `DEBUG` y definir `ALLOWED_HOSTS`.
